...
class Age_Gate_Public extends Age_Gate_Common {
	...
	public function is_restricted(){
		...
		if($this->settings['restrictions']['restriction_type'] === 'selected' && $this->settings['restrictions']['inherit_category'] && is_single() ){
				...
				if(wp_get_post_terms($this->id, $this->settings['advanced']['inherit_taxonomies'], ['meta_key' => '_age_gate-restrict'])){
					$restricted = true;
					$restricted = apply_filters('age_gate_inherited', $restricted, $this->meta, ['type' => 'inherited']);
					$this->restricted = $restricted;
					return;
				}
		}
		...
		if($this->settings['restrictions']['restriction_type'] === 'all' && $this->settings['restrictions']['inherit_category'] && is_single() ){
			...
			sort($this->settings['advanced']['inherit_taxonomies']);
				...
				if(wp_get_post_terms($this->id, $this->settings['advanced']['inherit_taxonomies'], ['meta_key' => '_age_gate-bypass'])){
					$restricted = false;
					$restricted = apply_filters('age_gate_inherited', $restricted, $this->meta, ['type' => 'inherited']);
					$this->restricted = $restricted;
					return;
  ...
  protected function _get_meta($id, $type = 'post')
  {
	...
	public function toolbar_link_to_mypage( $wp_admin_bar ) {
		...
		if(	$this->settings['advanced']['full_nav'] === 'off') return;
		...
		$toggle = (isset($_COOKIE['age_gate']) && $_COOKIE['age_gate'] ? 'show' : 'hide');
		$toggleText = (isset($_COOKIE['age_gate']) && $_COOKIE['age_gate'] ? __('Show Age Gate', 'age-gate'):__('Hide Age Gate', 'age-gate'));


		$args = array('id' => 'age-gate-toggle', 'title' => $toggleText, 'parent' => 'age-gate', 'href' => add_query_arg(
			['page' => 'age-gate', 'ag_switch' => $toggle, '_wpnonce' => wp_create_nonce( 'age-gate-toggle' )], admin_url('admin.php')));
